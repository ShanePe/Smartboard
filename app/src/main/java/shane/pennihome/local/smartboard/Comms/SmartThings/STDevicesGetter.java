...
public class STDevicesGetter extends AsyncTask<String, String, ComResult> {
    ...
    private boolean _includeRoutine;

    private ProcessCompleteListener<STDevicesGetter> _processCompleteListener;

    public STDevicesGetter(String uriRequest, boolean includeRoutine, ProcessCompleteListener<STDevicesGetter> processCompleteListener) {
        _uriRequest = uriRequest;
        _includeRoutine = includeRoutine;
    ...
    protected void onPreExecute() {
        ...
        pDialog.setMessage("Getting devices/routines from SmartThings ...");
    ...
    protected ComResult doInBackground(String... args) {               //DevicesGet
        ...
        try {
            ...
            coms.getJson(_uriRequest + "/switches", smartThingsTokenInfo.getToken(), new CommResponseListener() {
                @Override
            ...
            if (_includeRoutine) {
                coms.getJson(_uriRequest + "/routines", smartThingsTokenInfo.getToken(), new CommResponseListener() {
                    @Override
                    public void Process(JSONObject obj) {
                        routines.put(obj);
                    }
    ...
    protected void onPostExecute(ComResult result) {              //DevicesGet
        ...
        try {
            ...
            Collections.sort(_devices, new DeviceSorter());

            if (_includeRoutine) {
                _routines = new ArrayList<>();
            ...
            Collections.sort(_routines, new RoutineSorter());

            _success = true;
        } catch (Exception e) {
            Toast.makeText(Globals.getContext(), "Communication Request Error", Toast.LENGTH_SHORT).show();
            _success = false;
        }
