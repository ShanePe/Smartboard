...
final class AdapterImpl implements View.OnClickListener, View.OnLongClickListener {
  ...
  private final SparseArray<RowInfo> itemsPerRow = new SparseArray<>();
  ...
  void recalculateItemsPerRow() {
    ...
    asyncTask = new ProcessRowsTask(this);
  ...
  static class ProcessRowsTask extends AsyncTask<Void, Void, List<RowInfo>> {
    private final AdapterImpl mAdapter;
      ...
      ProcessRowsTask(AdapterImpl mAdapter) {
      ...
      for (int i = 0; i < mAdapter.agvAdapter.getActualItemCount(); i++) {
        try {
          itemsToAdd.add(new RowItem(i, mAdapter.agvAdapter.getItem(i)));
    ...
    @Override protected void onPostExecute(List<RowInfo> rows) {
      for (RowInfo row : rows) {
          mAdapter.itemsPerRow.put(mAdapter.getRowCount(), row);
      }

        mAdapter.agvAdapter.notifyDataSetChanged();
    }
    ...
    private List<RowInfo> calculateItemsPerRow(List<RowItem> itemsToAdd) {
      ...
      while (!itemsToAdd.isEmpty()) {
        RowInfo stuffThatFit = mAdapter.calculateItemsForRow(itemsToAdd);
        ...
        itemsToAdd.removeAll(itemsThatFit);
