...
StrongBuilderBase<T extends StrongBuilderBase, C>
    ...
    private final static String TOR_CHECK_URL = "https://check.torproject.org/api/ip";
    private final static String PROXY_HOST = "127.0.0.1";
    private final Context ctxt;
    SSLContext sslContext = null;
    private Proxy.Type proxyType;
    private boolean useWeakCiphers = false;
     ...
     */
    StrongBuilderBase(Context ctxt) {
    ...
    private int getHttpPort(Intent status) {
        if (status.getStringExtra(OrbotHelper.EXTRA_STATUS)
                .equals(OrbotHelper.STATUS_ON)) {
    ...
    Proxy buildProxy(Intent status) {
        Proxy result = null;
    ...
    private void checkTor(final Callback<C> callback, final Intent status,
                          final C connection) {
        new Thread() {
            @Override
            public void run() {
